...
def pairTransitionInputCode : Code := Code.prepend (Code.get 4) (Code.prepend (Code.get 5)
  (Code.prepend (Code.get 1) (Code.prepend (Code.get 0) (Code.drop 8))))

def pairReachCoefficient : Nat := 4*(30000+4*(40000+4*(10000+4*(20000+50000+1)+1)+1)+1)
def pairTransitionCoefficient : Nat := 4*(50000+4*(60000+4*(20000+4*(10000+90000+1)+1)+1)+1)
...
theorem pairTransitionInput_eval (cells : Bool → List Cell) (height bound bits first second : Nat) :
    pairTransitionInputCode.eval (pairValues bits first second (suffix cells height bound)) =
      pure (Arithmetic.input cells height bound first second) := by
  simp [pairTransitionInputCode,pairValues,suffix,Arithmetic.input]
...
theorem pairTransitionInput_fits (cells : Bool → List Cell) (height bound bits first second : Nat) :
    EvaluatorCodeFits pairTransitionInputCode (pairValues bits first second (suffix cells height bound))
      (Arithmetic.input cells height bound first second)
      (pairTransitionCoefficient*(encodedListSpace (pairValues bits first second (suffix cells height bound))+1)) := by
  ...
  have fit := prepend_linear (get_linear 4 values) (prepend_linear (get_linear 5 values)
    (prepend_linear (get_linear 1 values) (prepend_linear (get_linear 0 values) (drop_linear 8 values))))
  change EvaluatorCodeFits pairTransitionInputCode values (Arithmetic.input cells height bound first second)
