/-
Copyright (c) 2026 lean-trominoes contributors. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Erik Demaine, Stefan Langerman, GPT 5.6
-/
import LeanTrominoes.UnaryAlignedAddExecutionSupport
import LeanTrominoes.UnaryAlignedAddSecondFieldSteps

/-! # Second-field copying for aligned unary addition -/

noncomputable section

namespace LeanTrominoes
namespace UnaryAlignedAddMachine

open StateTransition Turing

def copySecond_evalsInTime (count : Nat) (tail : List UnarySymbol)
    (data : TapeData)
    (secondsEq : data.seconds =
      UnaryFieldEncoderMachine.unaryField count ++ tail) :
    EvalsToInTime machine.step (scanSecondFieldCfg data)
      (some (emitDelimiterCfg
        { data with
          seconds := tail
          outputReverse :=
            List.replicate count .unit ++ data.outputReverse }))
      (2 * count + 1) := by
  induction count generalizing data with
  | zero =>
      have step := step_scanSecondField_delimiter data tail
        (by simpa [UnaryFieldEncoderMachine.unaryField] using secondsEq)
      simpa using oneStep step
  | succ count induction =>
      have secondsHead : data.seconds = .unit ::
          (UnaryFieldEncoderMachine.unaryField count ++ tail) := by
        simpa [UnaryFieldEncoderMachine.unaryField,
          List.replicate_succ] using secondsEq
      let popped := oneStep
        (step_scanSecondField_unit data
          (UnaryFieldEncoderMachine.unaryField count ++ tail) secondsHead)
      let pushed := oneStep
        (step_pushSecondUnit
          { data with
            seconds := UnaryFieldEncoderMachine.unaryField count ++ tail })
      let nextData : TapeData :=
        { data with
          seconds := UnaryFieldEncoderMachine.unaryField count ++ tail
          outputReverse := .unit :: data.outputReverse }
      let rest := induction nextData rfl
      let firstTwo := EvalsToInTime.trans machine.step
        1 1 _ _ _ popped pushed
      let whole := EvalsToInTime.trans machine.step
        2 (2 * count + 1) _ _ _ firstTwo rest
      simpa [whole, nextData, List.replicate_succ, Nat.mul_add,
        replicate_unit_cons_comm, List.append_assoc,
        Nat.add_assoc] using whole

end UnaryAlignedAddMachine
end LeanTrominoes
