...
/-!
# Unit-elimination source-occurrence provenance

The positioned unit-elimination metadata and the logical occurrence-order
pairing traverse the same flattened clause blocks.  This file proves that
every inherited output incidence is paired with the exact source clause and
...
theorem clauseOriginalOccurrencePairs_mem_of_members
  ...
  · rcases rest with _ | ⟨second, rest⟩
    · simp [clauseClauses, auxiliary,
        PeriodicOneInThree.negate, liftLiteral]
          at generatedMember
    ...
    · rcases rest with _ | ⟨third, rest⟩
      · simp [clauseClauses, liftLiteral] at generatedMember
        rcases generatedMember with ⟨rfl, rfl⟩
        simp_all [clauseOriginalOccurrencePairs,
          mappedClauseOriginalOccurrencePairs,
          originalOccurrencePairIf, liftLiteral]
        aesop
...
theorem originalOccurrencePair_of_metadataLookup
    ...
    ((outputLiteral, outputStart + metadataIndex,
      ...
      (sourceLiteral, metadata.sourceClauseIndex,
        sourceLiteralIndex)) ∈
      PeriodicOneInThreeNoUnits.formulaOriginalOccurrencePairsFrom
  ...
  | cons clause rest induction =>
      ...
      · rw [List.getElem?_append_left inHead] at metadataLookup
        ...
        have metadataLocalIndex :
            metadata.localClauseIndex = metadataIndex :=
          clauseMetadataFor_lookup_localClauseIndex
        ...
        have erasedGeneratedMember :
          ...
          exact List.mem_zipIdx_iff_getElem?.mpr erasedLookup
        have localPair :=
          PeriodicOneInThreeNoUnits.clauseOriginalOccurrencePairs_mem_of_members
        ...
        rw [PeriodicOneInThreeNoUnits.formulaOriginalOccurrencePairsFrom]
...
theorem originalOccurrencePair_of_formulaMetadataLookup
  ...
  have paired :=
    originalOccurrencePair_of_metadataLookup
      ...
      outputAtom sourceAtom
  simpa [PeriodicOneInThreeNoUnits.formulaOriginalOccurrencePairs,
