...
import LeanTrominoes.DelimitedBinaryWordPrefixTrueCountLocalTimeBounds

/-! # Row-list time bound for prefix true counts -/
...
theorem rowsTime_le (rowIndex : Nat) (rows : List (List Bool)) :
  ...
  | cons row rows induction =>
      ...
      have tailBudget :
        ...
        have rowLengthMin : 2 ≤ rowLength := by simp [rowLength]
        simp [budget]
        omega
