/-
Copyright (c) 2026 lean-trominoes contributors. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Erik Demaine, Stefan Langerman, GPT 5.6
-/
import LeanTrominoes.UnarySuccessorEqualityFilterScanExecution

/-! # Complete execution of unary successor-equality filtering -/

noncomputable section

namespace LeanTrominoes
namespace UnarySuccessorEqualityFilterMachine

open StateTransition Turing

/-- Exact running time on a promised aligned input. -/
def totalTime (input : Input) : Nat :=
  2 * (outputEncoding input).length + 1 + scanTime input

/-- Exact execution from the separated input to the selected unary fields. -/
def execution (input : Input) :
    EvalsToInTime machine.step
      (scanLeftCfg ⟨encode input, [], [], [], [], [], [], []⟩)
      (some (haltCfg (outputEncoding input)))
      (totalTime input) := by
  let filteredData : TapeData :=
    ⟨[], [], [], [], [], [], (outputEncoding input).reverse, []⟩
  have scanRun := scan_evalsInTime input
  have scanRun' : EvalsToInTime machine.step
      (scanLeftCfg ⟨encode input, [], [], [], [], [], [], []⟩)
      (some (reverseOutputCfg filteredData)) (scanTime input) := by
    simpa [filteredData] using scanRun
  have reverseRun := reverseOutput_evalsInTime
    (outputEncoding input).reverse filteredData rfl
  have reverseRun' : EvalsToInTime machine.step
      (reverseOutputCfg filteredData)
      (some (haltCfg (outputEncoding input)))
      (2 * (outputEncoding input).length + 1) := by
    simpa [filteredData, haltCfg] using reverseRun
  have whole := EvalsToInTime.trans machine.step
    (scanTime input) (2 * (outputEncoding input).length + 1)
    _ _ _ scanRun' reverseRun'
  simpa [totalTime] using whole

end UnarySuccessorEqualityFilterMachine
end LeanTrominoes
