...
-/
import LeanTrominoes.PolyominoStripCycleAdapters
...
def pairTransitionCode : Code := Formula.transitionDecision.code.comp pairTransitionInputCode
...
def pairTransitionTotalCoefficient : Nat :=
  Formula.transitionSpaceConstant*(pairTransitionCoefficient+1)+pairTransitionCoefficient

def pairPredicate (cells : Bool → List Cell) (height bound bits first second : Nat) : Bool :=
  decide (first < 2^bits ∧ second < 2^bits) &&
    (Raw.check cells height bound first second &&
      divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits second first)
...
theorem pairTransition_eval (cells : Bool → List Cell) (height bound bits first second : Nat)
  ...
  simp [pairTransitionCode,pairTransitionInput_eval,Part.bind_eq_bind,
    Formula.transition_code_eval cells height bound first second bounded]
...
theorem pairReach_eval (cells : Bool → List Cell) (height bound bits first second : Nat)
    ...
    pairReachCode.eval (pairValues bits first second (suffix cells height bound)) =
      pure [(divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits second first).toNat] := by
...
theorem pair_eval (cells : Bool → List Cell) (height bound bits first second : Nat)
  ...
  let a := Raw.check cells height bound first second
  let b := divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits second first
...
theorem pair_fits (cells : Bool → List Cell) (height bound bits first second : Nat)
  ...
  have body : decide (first < 2^bits ∧ second < 2^bits) = true →
      EvaluatorCodeFits pairBodyCode values
        [(Raw.check cells height bound first second &&
          divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits second first).toNat]
        (pairBodyBudget bits space) := by
    intro hg
    have hg' := of_decide_eq_true hg
    have edge := comp_linear (Formula.transition_code_fits cells height bound first second bounded)
      (pairTransitionInput_fits cells height bound bits first second)
    have edge' : EvaluatorCodeFits pairTransitionCode values [(Raw.check cells height bound first second).toNat]
