...
-/
import LeanTrominoes.CompletionLStripStates
...
theorem exterior_completion (palette : Cell → Fin 24) (count : Int) (hn : 0 < count)
    ...
    Tromino.L.Completable (StripCaps.coreBand .L (36*count))ᶜ (exteriorPrescribed palette count) := by
  ...
  have region : {c | ∃ o : Selected, c ∈ stateTarget o.val (booleanOutside (fun _ => false) o.val)} =
      (StripCaps.coreBand .L (36*count))ᶜ := by
    ext c
    constructor
    · rintro ⟨o,hc⟩ inside
...
theorem plane_completion_of_core (palette : Cell → Fin 24) (count : Int) (hn : 0 < count)
    (blank : ∀ location, ¬ InBand count location → palette location = 0)
    (core : Tromino.L.Completable (StripCaps.coreBand .L (36*count)) (bandPrescribed palette count)) :
