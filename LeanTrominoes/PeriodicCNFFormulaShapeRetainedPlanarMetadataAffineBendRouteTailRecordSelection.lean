...
/-! # Exact finite selection of retained-bend Figure 9 records -/
...
theorem BendTemplate.descriptorPredicates_selectedRouteTailRecordBlock
...
theorem BendTemplate.routeTailRecordSelection_eq_nil_of_not_matches
    (shape : RouteShape) (template : BendTemplate)
    (pair : RouteDescriptor × RouteDescriptor)
...
theorem RouteShape.bendRouteTailRecordSelection_eq
    (shape : RouteShape)
    (pair : RouteDescriptor × RouteDescriptor)
    (sameEdge : pair.1.edgeIndex = pair.2.edgeIndex)
    (shapeMatches : shape.Matches pair.1)
  ...
  · apply List.flatMap_congr
    ...
    rw [← predicateListBlocks_eq]
    exact template.descriptorPredicates_selectedRouteTailRecordBlock
...
theorem RouteShape.bendRouteTailRecordSelection_eq_nil_of_not_matches
    (shape : RouteShape)
    (pair : RouteDescriptor × RouteDescriptor)
    (notMatches : ¬shape.Matches pair.1) :
    predicateListBlocks
  ...
  · apply List.flatMap_eq_nil_iff.mpr
    intro template templateMember
    rw [← predicateListBlocks_eq]
    exact template.routeTailRecordSelection_eq_nil_of_not_matches
      shape pair notMatches
  · intro template _templateMember
    simp [BendTemplate.descriptorPredicates,
      bendTemplateRouteTailRecordBlocks]

theorem affineBaseBendRouteTailRecordBlock_eq_of_matches
  ...
  · rw [flatMap_eq_of_unique
      allRouteShapes
      (fun shape =>
        selectTruthBlocks shape.bendRouteTailRecordBlocks
    ...
    · rw [← predicateListBlocks_eq]
      exact selectedShape.bendRouteTailRecordSelection_eq pair sameEdge
        shapeMatches incomingGenuine outgoingGenuine
    · intro shape _shapeMember shapeNe
      rw [← predicateListBlocks_eq]
      apply shape.bendRouteTailRecordSelection_eq_nil_of_not_matches
      intro otherMatches
