...
theorem recover_tromino_of_grid {n : Nat} (hn : 96 ≤ n)
    ...
    (tiling : IsTiling (pairTiles PlusRefinement.bumpy (tile n holes)) Set.univ ps)
  ...
  have exact_grid : {p : Placement Unit | p.tag true ∈ ps} = gridPlacements n := by
    ...
    · intro p hp
      ...
      rw [← carrier]
      by_contra outside
      obtain ⟨q, hq, hqc⟩ := canonical.exists_cover outside
  ...
  have background : IsTiling (fun _ : Unit => tile n holes)
      (PlusRefinement.region original)ᶜ {p | p.tag true ∈ ps} := by
    rw [exact_grid, ← carrier]
    exact canonical
  exact (PlusRefinement.bumpy_tileable_refinement_iff original).mp
    (tileable_left_of_background _ _ _ ps tiling background)
