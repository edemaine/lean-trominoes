...
-/
import LeanTrominoes.CompletionLStripCapped
import LeanTrominoes.CompletionHorizontalPrefill

/-! # A finite horizontal motif for a band of L-bricks -/
...
theorem band_prescribed_motif (palette : Cell → Fin 24) (count : Int) (f : Finset Cell) :
    f ∈ bandPrescribed palette count ↔ ∃ location : Cell, InBand count location ∧
      ∃ p ∈ motif (palette location), f = (p.shift (origin location)).cells (fun _ => Tromino.L.cells) := by
  constructor
  · rintro ⟨o,hb,p,hp,eq⟩
...
theorem origin_horizontal_repeat (c : Cell) (period i : Int) :
    origin (c.1+i*period,c.2) = Cell.add (i*(24*period),0) (origin c) := by
...
theorem bandMotif_prescribed (period count : Nat) (periodPositive : 0 < period) (palette : Cell → Fin 24)
    ...
    HorizontalPrefill.prescribed .L (24*period) (bandMotif period count palette) =
