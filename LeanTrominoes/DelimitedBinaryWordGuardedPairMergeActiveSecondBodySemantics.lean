...
/-! # Active second-body scan semantics for guarded word-pair merging -/
...
theorem scan_activeSecondBody_bits_wordEnd
    (bits : List Bool) (suffix : List Token) :
    scan transition (.secondBody true)
        (bits.map Token.bit ++ .wordEnd :: suffix) =
      let rest := scan transition .firstStart suffix
      (rest.1, bits.map Token.bit ++ .wordEnd :: rest.2) := by
  induction bits with
  | nil => rfl
