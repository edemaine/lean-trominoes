...
/-!
# Separation of complete final cyclic-round route occurrences
...
theorem ContinuousPlanarPresentation.reversedFinalNormalizationTemplateOccurrences_avoid
  ...
  dsimp only
  have forward :=
    presentation.finalNormalizationTemplateOccurrences_avoid
...
boundaries for one lifted final cyclic-round route. -/
theorem ContinuousPlanarPresentation.finalNormalizationOccurrence_junctions
    {problem : PeriodicThreeDM}
    (presentation : problem.ContinuousPlanarPresentation)
    (wellFormed : problem.IsWellFormed)
    (degree : problem.DegreeTwoOrThree)
...
after the final cyclic normalization splice. -/
theorem ContinuousPlanarPresentation.finalNormalizationRouteOccurrences_avoidEachOther
    {problem : PeriodicThreeDM}
    (presentation : problem.ContinuousPlanarPresentation)
    (wellFormed : problem.IsWellFormed)
    (degree : problem.DegreeTwoOrThree)
    ...
    let planar := presentation.toPlanarPresentation
    RoutesAvoidEachOther
      (planar.finalNormalizationRouteOccurrence firstEdge firstTranslate)
      (planar.finalNormalizationRouteOccurrence secondEdge secondTranslate) := by
  dsimp only
  let planar := presentation.toPlanarPresentation
  ...
  let firstSource := planar.finalNormalizationTemplateOccurrence
    firstSourceEndpoint firstTranslate
  let firstMiddle := planar.finalNormalizationCorridorOccurrence
    firstEdge firstTranslate
  let firstTarget :=
    (planar.finalNormalizationTemplateOccurrence
      firstTargetEndpoint firstTranslate).reverse
  let secondSource := planar.finalNormalizationTemplateOccurrence
    secondSourceEndpoint secondTranslate
  let secondMiddle := planar.finalNormalizationCorridorOccurrence
    secondEdge secondTranslate
  let secondTarget :=
    (planar.finalNormalizationTemplateOccurrence
  ...
  have firstSourceSecondMiddle :
    ...
    exact
      presentation.finalNormalizationTemplateOccurrence_strictlyAvoids_corridorOccurrence
  ...
  have middlesStrict :
    ...
    exact presentation.finalNormalizationCorridorOccurrences_strictlyAvoid
  ...
  have prefixes :=
    sourcePair.1.join_tails_of_prefixes_meet_only_at_heads
      ...
      secondSourceLast secondMiddleHead
  have targetPair :=
    presentation.reversedFinalNormalizationTemplateOccurrences_avoid
  ...
  rw [planar.finalNormalizationRouteOccurrence_eq_threePieces,
    planar.finalNormalizationRouteOccurrence_eq_threePieces,
