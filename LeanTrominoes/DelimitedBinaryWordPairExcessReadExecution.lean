...
/-! # Word-reading executions for unary word-pair excesses -/
...
def readFirst_evalsInTime (keepFirst : Bool) (word : List Bool)
    (tail : List Token) (data : TapeData)
    (inputEq : data.input = word.map Token.firstBit ++ .middle :: tail) :
    EvalsToInTime (TM2.step (program keepFirst)) (readFirstCfg data)
      (some (readSecondCfg
        { data with
          input := tail
  ...
  | nil =>
      have step := oneStep
        (step_readFirst_middle keepFirst data tail inputEq)
  ...
  | cons bit word induction =>
      ...
      have first := oneStep (step_readFirst_bit keepFirst data bit
...
def readSecond_evalsInTime (keepFirst : Bool) (word : List Bool)
    (tail : List Token) (data : TapeData)
    (inputEq : data.input = word.map Token.secondBit ++ .pairEnd :: tail) :
    EvalsToInTime (TM2.step (program keepFirst)) (readSecondCfg data)
      (some (cancelCfg
        { data with
          input := tail
  ...
  | nil =>
      have step := oneStep
        (step_readSecond_pairEnd keepFirst data tail inputEq)
      convert step using 1 <;> simp
  | cons bit word induction =>
      let nextData :=
        { data with
      ...
      have first := oneStep (step_readSecond_bit keepFirst data bit
