...
def preparedSymbols (source : List Bool) : List UnarySymbol :=
  List.replicate 7 .unit ++ (units source ++ [.delimiter]) ++ source.flatMap fieldBlock
...
theorem units_fields (values : List Nat) : units (Theorem55StripEncoding.unaryFields values) = List.replicate values.sum UnaryFieldEncoderMachine.Symbol.unit := by
  induction values with
  | nil => rfl
  | cons n values ih =>
    change units ((List.replicate n true ++ [false]) ++ Theorem55StripEncoding.unaryFields values) = _
...
theorem fieldBlock_fields (values : List Nat) :
    (Theorem55StripEncoding.unaryFields values).flatMap fieldBlock = UnaryFieldEncoderMachine.unaryFields values := by
  induction values with
  | nil => rfl
  | cons n values ih =>
    change ((List.replicate n true ++ [false]) ++ Theorem55StripEncoding.unaryFields values).flatMap fieldBlock = _
    rw [List.flatMap_append,fieldBlock_field,ih]
    rfl

theorem preparedSymbols_correct (input : Theorem55.StripInput) :
    preparedSymbols (Theorem55StripEncoding.finEncoding.encode input) = UnaryFieldEncoderMachine.unaryFields (fields input) := by
  change preparedSymbols (Theorem55StripEncoding.unaryFields (Theorem55StripEncoding.fields input)) = _
...
noncomputable def preparedSymbols_compiler : TM2ComputableInPolyTime id id preparedSymbols := by
  let frontCompiler := TM2ConstantValueCompiler.computableInPolyTime (Input := List Bool) id id (List.replicate 7 UnaryFieldEncoderMachine.Symbol.unit)
...
noncomputable def unaryPreparation_compiler :
    TM2ComputableInPolyTime Theorem55StripEncoding.finEncoding.encode UnaryFieldEncoderMachine.unaryFields fields := by
  let raw := TM2PolyTimeInputEncodingTransport.of_prepare Theorem55StripEncoding.finEncoding.encode
...
noncomputable def nativePreparation_compiler :
    TM2ComputableInPolyTime Theorem55StripEncoding.finEncoding.encode Turing.PartrecToTM2.trList fields := by
