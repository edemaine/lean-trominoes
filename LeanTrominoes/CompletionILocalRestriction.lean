...
import LeanTrominoes.CompletionINoCrossing
import LeanTrominoes.TrominoCompletionRestriction

/-! # Restricting arbitrary plane completions to individual I subbricks -/
...
def atomPrescribed (location : Cell) (entry : Atom × Cell) : Set (Finset Cell) :=
  {f | ∃ p ∈ entry.1.motif,
    f = (p.shift (atomOffset location entry)).cells (fun _ => Tromino.I.cells)}
...
theorem completed_tile_contained (palette : Cell → Fin 24) (location : Cell)
  ...
  · have avoids : ∀ d ∈ f, d ∉ globalFilled palette := by
    ...
    have notFixed : c ∉ entry.1.fixed := by
      intro fixed
      exact avoids _ covers (atom_barrier_global palette location he fixed)
    ...
    exact residual_tile_contained palette location he h retained hf avoids core covers
...
theorem local_completion_of_global (palette : Cell → Fin 24) (location : Cell)
    {entry : Atom × Cell} (he : entry ∈ layout (palette location))
    (global : Tromino.I.Completable Set.univ (globalPrescribed palette)) :
    ∃ outside : Finset Cell,
      outside ⊆ entry.1.boundary.image (Cell.add (atomOffset location entry)) ∧
      Tromino.I.Completable
