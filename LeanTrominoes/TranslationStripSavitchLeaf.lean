...
import LeanTrominoes.TranslationStripExpr

/-! # Orientation-restricted strip SavitchLeaf certificates
...
def baseExpr : Expr := .ite (eqE (var 2) (var 3)) 1 TranslationStrip.Formula.transitionDecision
...
theorem baseExpr_noPower : baseExpr.noPower = true := by
  simp [baseExpr,eqE,Expr.noPower,TranslationStrip.Formula.transitionDecision_noPower]
...
theorem baseExpr_eval (cells : Bool → List Cell) (height bound first last : Nat)
    ...
    baseExpr.eval (Arithmetic.input cells height bound first last) =
      divideBoolTag (decide (first=last) || TranslationStrip.check cells height bound first last) := by
  ...
  change (if (eqE (var 2) (var 3)).eval (Arithmetic.input cells height bound first last) = 0 then
    TranslationStrip.Formula.transitionDecision.eval (Arithmetic.input cells height bound first last) else 1) = _
  rw [he,TranslationStrip.Formula.transitionDecision_eval cells height bound first last bounded]
  by_cases h : first=last
  · simp [h,divideBoolTag]
  · simp only [h,if_false,ite_true,decide_false,Bool.false_or]
    cases TranslationStrip.check cells height bound first last <;> rfl
...
theorem base_eval (cells : Bool → List Cell) (height bound : Nat)
    ...
    baseCode.eval (divideEvalProgramList context count state ++ suffix cells height bound) =
      pure [divideBoolTag (decide (state.query.first=state.query.last) ||
        TranslationStrip.check cells height bound state.query.first state.query.last)] := by
...
theorem base_fits (cells : Bool → List Cell) (height bound : Nat)
    ...
    EvaluatorCodeFits baseCode (GenericSavitchStep.flatProgramList (suffix cells height bound) context count state)
      [divideBoolTag (decide (state.query.first=state.query.last) || TranslationStrip.check cells height bound state.query.first state.query.last)]
