...
-/
import LeanTrominoes.UnaryRotatedRanksInterface
import LeanTrominoes.UnaryRotatedRanksTimeBound

/-! # Polynomial-time unary rotated ranks -/
...
noncomputable def timePolynomial : Polynomial Nat :=
  Polynomial.C 10 * (Polynomial.X + Polynomial.C 1)

@[simp] theorem timePolynomial_eval (length : Nat) :
    timePolynomial.eval length = 10 * (length + 1) := by
  simp [timePolynomial, Polynomial.eval_mul, Polynomial.eval_add]

/-- Rotating aligned unary ranks within positive-size groups is computable in
linear time in the complete separated encoding. -/
noncomputable def computableInPolyTime :
    @TM2ComputableInPolyTime
      Input (List Nat) InputSymbol UnarySymbol
      encode UnaryFieldEncoderMachine.unaryFields
      (fun input => rotatedRanks input.ranks input.sizes) where
  tm := machine
