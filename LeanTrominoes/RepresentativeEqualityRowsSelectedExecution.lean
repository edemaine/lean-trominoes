...
-/
import LeanTrominoes.RepresentativeEqualityRowsForwardExecution
import LeanTrominoes.RepresentativeEqualityRowsOutputTransferExecution

/-! # Complete selected-row transfer -/
...
/-- Preserve a selected row on the reverse-output stack. -/
def moveSelected_evalsInTime (tokens : List Token) (data : TapeData)
  ...
  have second := moveSelectedToOutput_evalsInTime tokens.reverse forwarded rfl
  have second' :
