...
-/
import LeanTrominoes.CompletionICanonicalConnectors
...
theorem equal_propagates_proper (outside : Finset Cell) (subset : outside ⊆ minorBoundary)
    (h : IGuardedEqBoundary.pattern.Completable outside) :
  ...
  rw [i_guarded_eq_boundary] at h
  have checked : ∀ j : Fin 16, (j = 3 ∨ j = 6 ∨ j = 9 ∨ j = 12) →
      (LocalProper (IGuardedEqBoundary.outside j) (0,0) ↔ LocalProper (IGuardedEqBoundary.outside j) (0,1)) := by decide +kernel
  exact checked i h
