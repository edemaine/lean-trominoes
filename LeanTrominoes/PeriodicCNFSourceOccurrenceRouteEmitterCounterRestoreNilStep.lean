...
/-! # Empty counter-restoration step for source-occurrence route emission -/

namespace LeanTrominoes.PeriodicCNF.SourceOccurrenceRouteEmitterMachine

open Turing

theorem step_restoreCounter_nil (stage : CounterStage) (cursor : Cursor)
    (data : TapeData) (scratchEq : data.scratch = []) :
    machine.step (restoreCounterCfg stage cursor data) =
      some (afterCounterCfg stage cursor { data with scratch := [] }) := by
