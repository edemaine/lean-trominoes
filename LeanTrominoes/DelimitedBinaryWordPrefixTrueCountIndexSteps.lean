...
import LeanTrominoes.DelimitedBinaryWordPrefixTrueCountTapeUpdates

/-! # Index transitions for row-prefix true counts -/
...
theorem step_scanStart_nil (data : TapeData) (inputEq : data.input = []) :
  ...
  subst input
  simp [TM2.step, program, scanStartCfg, clearRowIndexCfg, idleCfg,
...
theorem step_scanStart_cons (data : TapeData) (token : Token)
    ...
    TM2.step program (scanStartCfg data) =
      some (copyIndexCfg { data with input := tail }) := by
  rcases data with
    ⟨input, rowIndex, indexRestore, prefixCountdown,
      outputReverse, output⟩
  ...
  subst input
  cases token <;>
    simp [TM2.step, program, scanStartCfg, copyIndexCfg, idleCfg,
      cfg, tapes, setToken, tokenIsNone, clearToken, initialState]
...
theorem step_copyIndex_nil (data : TapeData)
  ...
  subst rowIndex
  simp [TM2.step, program, copyIndexCfg, restoreIndexCfg, idleCfg,
...
theorem step_copyIndex_cons (data : TapeData) (tail : List Unit)
  ...
  subst rowIndex
  simp [TM2.step, program, copyIndexCfg, idleCfg, cfg, tapes,
...
theorem step_restoreIndex_nil (data : TapeData)
    ...
    TM2.step program (restoreIndexCfg data) =
      some (scanPrefixCfg { data with indexRestore := [] }) := by
  rcases data with
    ⟨input, rowIndex, indexRestore, prefixCountdown,
      outputReverse, output⟩
  change indexRestore = [] at restoreEq
  subst indexRestore
  simp [TM2.step, program, restoreIndexCfg, scanPrefixCfg, idleCfg,
    cfg, tapes, setPresent, isPresent, clearPresent, initialState]
...
theorem step_restoreIndex_cons (data : TapeData) (tail : List Unit)
  ...
  subst indexRestore
  simp [TM2.step, program, restoreIndexCfg, idleCfg, cfg, tapes,
    setPresent, isPresent, clearPresent, initialState]
