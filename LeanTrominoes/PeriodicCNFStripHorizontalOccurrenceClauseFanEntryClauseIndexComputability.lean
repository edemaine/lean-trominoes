...
/-! # Computability of clause indices at proof-erased entries -/
...
theorem horizontalOccurrenceClauseEntryClauseIndexComputed_primrec :
    Primrec horizontalOccurrenceClauseEntryClauseIndexComputed := by
  ...
  exact (Primrec.option_casesOn
    ...
    (Primrec.const 0) selected.to₂).of_eq fun input => by
      cases found : horizontalOccurrenceClauseEntryLookupComputed input <;>
        simp [horizontalOccurrenceClauseEntryClauseIndexComputed, found]
