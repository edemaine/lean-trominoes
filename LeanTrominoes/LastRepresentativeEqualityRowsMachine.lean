...
/-! # Finite machine filtering last-representative equality rows -/
...
def inspectSuffixToken (state : State) : State :=
  { state with
    token := none
    present := false
    representative :=
  ...
  | .restoreIndex =>
      .pop .indexRestore setPresent
        (.branch isPresent
          (.push .rowIndex (fun _ => ())
            (.load clearPresent (.goto fun _ => .restoreIndex)))
          (.load clearPresent (.goto fun _ => .skipPrefix)))
  | .skipPrefix =>
        ...
        (.branch tokenIsNone
          ...
          (.branch tokenIsEnd
            (.push .rowReverse tokenFromState
              (.load clearToken (.goto fun _ => .finishRow)))
            (.push .rowReverse tokenFromState
              (.load inspectSuffixToken (.goto fun _ => .scanSuffix)))))
  | .clearPrefix =>
      .pop .prefixCountdown setPresent
        (.branch isPresent
...
def skipPrefixCfg (data : TapeData) := idleCfg .skipPrefix data
def skipDiagonalCfg (data : TapeData) := idleCfg .skipDiagonal data
def scanSuffixCfg (representative : Bool) (data : TapeData) :=
  cfg .scanSuffix ⟨none, false, representative⟩ data
def clearPrefixCfg (data : TapeData) := idleCfg .clearPrefix data
