...
import LeanTrominoes.PeriodicOrthocrossingCarrierCrossingPointFieldRankSemantics
...
/-! # Numeric semantics of crossing-point representative lookup -/
...
signed crossing-point magnitude over the stably deduplicated rank data. -/
theorem values_numericRouteDescriptors
  ...
  have nodeAligned :=
    CarrierCrossingPointCandidateFieldStream.values_forall₂_paddedCarrierNodeCandidateStream
      field descriptors
  have codeAligned : List.Forall₂
      (fun candidate value => ∀ code,
        candidate.value = some code → value = nodeValue field code.node)
      codeCandidates
      (CarrierCrossingPointCandidateFieldStream.values
        field descriptors) := by
    exact forall₂_mapActiveValue_code nodeCandidates
      (CarrierCrossingPointCandidateFieldStream.values field descriptors)
  ...
  have lookupEq :
      LastTrueUnaryValueLookupMachine.lookups
          ...
          (CarrierCrossingPointCandidateFieldStream.valuesWithSentinel
            field descriptors) =
        (codeCandidates.filterMap Candidate.value).dedup.map
          (fun code => nodeValue field code.node) := by
    unfold CarrierCrossingPointCandidateFieldStream.valuesWithSentinel
