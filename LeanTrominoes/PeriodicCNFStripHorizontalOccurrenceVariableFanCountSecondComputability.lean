...
/-! # Computability of second variable-fan slot activity -/
...
theorem horizontalOccurrenceVariableRibbonCountSecondComputed_primrec :
    PrimrecPred fun input : HorizontalVariableRibbonFanInput =>
      (horizontalOccurrenceVariableRibbonLookupComputed
        (input, VariableSiteSlot.second)).isSome :=
  horizontalOccurrenceVariableRibbonSlotActiveComputed_primrec.comp
    (Primrec.pair Primrec.id (Primrec.const VariableSiteSlot.second))

theorem horizontalOccurrenceVariableRibbonCountSecondBoolComputed_primrec :
