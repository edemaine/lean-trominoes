...
-/
import LeanTrominoes.CompletionIBrickBooleanWitness
...
def tableCore (i : Fin 24) (v : BrickValues) (p : Fin 4) : Bool :=
  coreSignal i (fun p => if h : p.val < 2 then v ⟨p.val,h⟩ 2 else v ⟨p.val - 2,by omega⟩ 4) p
...
theorem table_network_witness (i : Fin 24) (v : BrickValues)
    (h : BrickBooleanRelation i v) : NetworkWitness i (tableExternal v) (tableCore i v) := by
  ...
  all_goals
    ...
    simp [BrickBooleanRelation,layout,upperOuter,lowerOuter,inner,active,positive,bit,
      entryMicroOffset,Atom.BooleanRelation,Cell.add,tableValue,tableExternal,tableCore,coreSignal,literal,
...
theorem network_of_brick_table (i : Fin 24) (v : BrickValues)
  ...
  ⟨tableCore i v,table_network_witness i v h⟩
