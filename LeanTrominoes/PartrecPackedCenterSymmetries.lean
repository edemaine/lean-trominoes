...
/-!
# Packed center containment across all symmetries

The center-containment condition at one motif base is the conjunction of the
eight fixed square-symmetry candidate implications.  This file folds those
already explicit candidate programs into one explicit program and proves that
it computes the semantic `List.all` used by the packed frontier definition.
-/
...
/-- Conjoin the center-containment candidate programs indexed by a fixed list
of square symmetries. -/
def packedCenterSymmetryListInsideCode
    (tromino : Tromino) : List SquareSymmetry -> Code
  | [] => one
  | symmetry :: symmetries =>
      boolAnd
        (packedCenterCandidateInsideCode tromino symmetry)
        (packedCenterSymmetryListInsideCode tromino symmetries)

theorem packedCenterSymmetryListInsideCode_eval_semantic
    (tromino : Tromino) (symmetries : List SquareSymmetry)
    (periodicStrip : PeriodicStrip)
    (wellFormed : periodicStrip.IsWellFormed)
    (packed : PackedWindowState) (base : Cell) :
    (packedCenterSymmetryListInsideCode tromino symmetries).eval
  ...
  | nil =>
      simp [packedCenterSymmetryListInsideCode]
  | cons symmetry symmetries induction =>
      have headRun := packedCenterCandidateInsideCode_eval_semantic
        tromino symmetry periodicStrip wellFormed packed base
      have tailRun := induction
      simpa [packedCenterSymmetryListInsideCode] using
...
/-- Check every one of the eight square-symmetry center candidates at one
motif base. -/
def packedCenterAllSymmetriesInsideCode
    (tromino : Tromino) : Code :=
  packedCenterSymmetryListInsideCode tromino
    TrominoAssignment.squareSymmetryList

/-- The eight-way conjunction is exactly the symmetry fold appearing in the
semantic packed center-containment predicate. -/
@[simp]
theorem packedCenterAllSymmetriesInsideCode_eval_semantic
    (tromino : Tromino)
    (periodicStrip : PeriodicStrip)
    (wellFormed : periodicStrip.IsWellFormed)
    (packed : PackedWindowState) (base : Cell) :
    (packedCenterAllSymmetriesInsideCode tromino).eval
      ...
      pure [(TrominoAssignment.squareSymmetryList.all fun symmetry =>
        packed.centerSymmetryInsideBool tromino periodicStrip
          base symmetry).toNat] := by
  exact packedCenterSymmetryListInsideCode_eval_semantic
