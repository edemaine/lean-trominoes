...
/-! # Physical direct retained-bend terminal-radial stream -/
...
noncomputable local instance directBaseBendRadialStreamStackFintype
    (stack : decider.tm.K) : Fintype (decider.tm.Γ stack) :=
  decider.stackAlphabetFinite stack

/-- Physical composition through tagged descriptor pairs and the finite
bend-port selector. -/
opaque directSourceBaseBendTerminalRadialStreamComputableInPolyTime :
    TM2ComputableInPolyTime id id
      (directSourceBaseBendTerminalRadialStream decider) := by
  ...
  exact TM2CompositionMachine.computableInPolyTime
    ...
    RouteDescriptorPairAffine.affineBaseBendTerminalRadialStreamComputableInPolyTime
