...
-/
import LeanTrominoes.CompletionLStripBandCompiler
...
def compileStrip (period count : Nat) (palette : Cell → Fin 24) : PeriodicStripTrominoPrefill where
  ...
  motif := ((bandMotif period count palette).map fun p => p.shift (0,2)) ++
    (StripCaps.expandedCap .L true period ++
      (StripCaps.expandedCap .L false period).map fun p => p.shift (0,36*(count:Int)+2))
...
theorem compileStrip_prescribed (period count : Nat) (hp : 0 < period) (palette : Cell → Fin 24)
    ...
    (compileStrip period count palette).periodic.prescribed .L =
      (fun f => f.image (Cell.add (0,2))) ''
        (bandPrescribed palette count ∪ StripCaps.shellPrefill .L (36*count)) := by
  rw [HorizontalPrefill.strip_prescribed]
  have top := StripCaps.expandedCap_prescribed .L true period hp
  have bottom := StripCaps.expandedCap_prescribed .L false period hp
  change HorizontalPrefill.prescribed .L (24*(period:Int)) _ = _ at top bottom
  change HorizontalPrefill.prescribed .L (24*(period:Int)) _ = _
...
theorem compileStrip_region (period count : Nat) (palette : Cell → Fin 24) :
    (compileStrip period count palette).region =
      Cell.add (0,2) '' StripCaps.cappedRegion (36*(count:Int)) := by
  ext c
...
theorem compileStrip_correct (period count : Nat) (hp : 0 < period) (hn : 0 < count)
    ...
    PeriodicStripTrominoPrefill.problem .L (compileStrip period count palette) ↔
