...
-/
import LeanTrominoes.DelimitedBinaryWordPairEqualityExecution

/-! # Linear clock bound for delimited binary-word equality -/
...
theorem totalTime_le (input : Input) :
  ...
  change pairsTime pairs +
      reverseTime (List.map (fun pair => decide (pair.1 = pair.2)) pairs).reverse ≤
