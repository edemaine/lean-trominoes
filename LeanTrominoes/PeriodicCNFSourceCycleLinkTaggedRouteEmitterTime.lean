...
-/
import LeanTrominoes.PeriodicCNFSourceCycleLinkTaggedRouteEmitterInterface
import LeanTrominoes.PeriodicCNFSourceCycleLinkTaggedRouteEmitterTimeBound

/-! # Polynomial-time tagged cycle-link route emission -/
...
noncomputable def timePolynomial : Polynomial Nat :=
  Polynomial.C 126 * Polynomial.X ^ 2

@[simp] theorem timePolynomial_eval (length : Nat) :
    timePolynomial.eval length = 126 * length ^ 2 := by
  simp [timePolynomial, Polynomial.eval_mul, Polynomial.eval_pow]

/-- Emit all tagged source cycle-link route records in quadratic time in the
complete separated input encoding. -/
noncomputable def computableInPolyTime :
    @TM2ComputableInPolyTime
      SourceCycleLinkTaggedRouteEmitter.Input (List OutputToken)
      InputSymbol OutputToken SourceCycleLinkTaggedRouteEmitter.encode id
      SourceCycleLinkTaggedRouteEmitter.emit where
  tm := machine
  ...
  outputsFun input := by
    ...
    have run' : TM2OutputsInTime machine
        ...
        (some (List.map (Equiv.refl OutputToken).invFun
          (id (SourceCycleLinkTaggedRouteEmitter.emit input))))
        (totalTime input) := by
