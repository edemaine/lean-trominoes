...
-/
import LeanTrominoes.LastRepresentativeEqualityRowsTapeUpdates

/-! # Index steps of the last-representative row machine -/
...
theorem step_restoreIndex_nil (data : TapeData)
    ...
    TM2.step program (restoreIndexCfg data) =
      some (skipPrefixCfg { data with indexRestore := [] }) := by
  rcases data with
    ⟨input, rowIndex, indexRestore, prefixCountdown, rowReverse,
      rowForward, outputReverse, output⟩
  change indexRestore = [] at restoreEq
  subst indexRestore
  simp [TM2.step, program, restoreIndexCfg, skipPrefixCfg, idleCfg, cfg,
    tapes, setPresent, isPresent, clearPresent, initialState]
