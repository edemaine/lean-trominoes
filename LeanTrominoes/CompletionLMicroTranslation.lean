...
-/
import LeanTrominoes.CompletionLMicroGroups
...
theorem micro_translation (delta i c : Cell) :
  ...
  have hx : (Cell.add (microOrigin delta) c).1 - 12 * (Cell.add delta i).1 =
      c.1 - 12 * i.1 := by dsimp [Cell.add,microOrigin]; omega
  have hy : (Cell.add (microOrigin delta) c).2 - 6 * (Cell.add delta i).2 =
      c.2 - 6 * i.2 := by dsimp [Cell.add,microOrigin]; omega
  rw [hx,hy]
