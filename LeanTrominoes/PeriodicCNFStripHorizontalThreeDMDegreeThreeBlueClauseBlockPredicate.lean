...
/-! # Blue degree-three filter on one clause block -/
...
theorem horizontalThreeDMBlueClauseBlock_filter_of_arity
    ...
    ([BlueElement.clauseInternal tagged.2] ++
        allTerminalGroups.map
          (BlueElement.clauseTerminal tagged.2)).filter
        (horizontalThreeDMBlueElementDegreeThree typed) =
  ...
  · simp [horizontalThreeDMBlueElementDegreeThree, allTerminalGroups,
