...
/-- Shared clause heads do not obstruct strict outer-fan separation when
the retained source prefixes are not both singletons.  The finite source
drawing then strictly separates the two discarded terminal rectangles even
though the original routes may meet at that common head. -/
theorem
