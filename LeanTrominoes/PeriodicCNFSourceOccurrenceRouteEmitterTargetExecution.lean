...
import LeanTrominoes.PeriodicCNFSourceOccurrenceRouteEmitterTargetUnitStep
import LeanTrominoes.PeriodicCNFSourceOccurrenceRouteEmitterTargetDelimiterStep

/-! # Target-field execution for source-occurrence route emission -/
...
noncomputable def scanTarget_evalsInTime (count : Nat)
    (cursor : Cursor) (data : TapeData) (remaining : List UnarySymbol)
    ...
    EvalsToInTime machine.step
      ...
      (some (finishRecordCfg cursor.literalIndex cursor.currentNext
        ...
        { data with
          targets := remaining
          outputReverse :=
            .atomEnd :: List.replicate count .atomUnit ++
              data.outputReverse }))
      (count + 1) := by
  induction count generalizing data with
  | zero =>
      have step := oneStep
        (step_scanTarget_delimiter cursor data remaining targetsEq)
      simpa using step
  | succ count induction =>
      ...
      have first := oneStep
        (step_scanTarget_unit cursor data
