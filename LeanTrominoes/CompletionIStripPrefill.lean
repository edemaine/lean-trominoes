...
-/
import LeanTrominoes.CompletionIStripCut
...
theorem motif_core_bounds : ∀ i : Fin 24, ∀ p ∈ motif i, ∀ c ∈ p.cells (fun _ => Tromino.I.cells),
    0 ≤ c.2 ∧ c.2 ≤ 162 ∧ (c.2 = 0 → 9 ≤ c.1%18) ∧ (c.2 = 162 → c.1%18 < 9) := by
  decide +kernel

theorem band_prescribed_inside (palette : Cell → Fin 24) (count : Int)
    (f : Finset Cell) (hf : f ∈ bandPrescribed palette count) :
    ∀ c ∈ f, c ∈ StripCaps.coreBand .I (162*count) := by
  ...
  intro c hc
  rw [eq] at hc
  have shifted : (p.shift (atomOffset o.location o.entry)).cells (fun _ => Tromino.I.cells) =
      ((p.shift o.entry.2).shift (origin o.location)).cells (fun _ => Tromino.I.cells) := by
    congr 1
