...
/-! # Compute the graph size and initialize exact-one cycle search -/
...
def result (f : PeriodicCNF Nat) : Bool :=
  cycleSearchIndexDFSBoolAtDepth (2^bits f) (bits f) (ExactOneFieldPredicate.check f)

theorem result_correct (f : PeriodicCNF Nat) : result f = true ↔ PeriodicExactOneCNF.LocalOneDimensionalThreeSAT f := by
  rw [result,cycleSearchIndexDFSBoolAtDepth_eq]
  exact (cycleSearchIndexBoolAtDepth_eq_true_iff (2^bits f) (bits f) (ExactOneFieldPredicate.check f) le_rfl).trans
    (ExactOneFieldPredicate.cycle_iff f)
