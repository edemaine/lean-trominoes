...
/-- Copy the unary row index onto both temporary stacks. -/
def copyIndex_evalsInTime (tokens : List Unit) (data : TapeData)
  ...
  | nil =>
      have step := oneStep (step_copyIndex_nil data indexEq)
      simpa using step
  | cons marker tokens induction =>
      ...
      have first := oneStep (step_copyIndex_cons data tokens indexEq)
