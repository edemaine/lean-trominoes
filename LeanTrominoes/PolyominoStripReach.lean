...
/-! # A complete certified variable-tile reachability program -/
...
theorem reach_eval (cells : Bool → List Cell) (height bound bits first last : Nat)
    ...
    reachCode.eval (reachRequest bits first last (suffix cells height bound)) =
      pure [divideBoolTag (divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits first last)] := by
  ...
  have loopRun := DivideEvalPartrec.flatIterate_stepCode_eval_suffix baseCode 0 (2^bits)
    (Raw.check cells height bound) (suffix cells height bound)
...
theorem reach_fits (cells : Bool → List Cell) (height bound bits first last : Nat)
    ...
    EvaluatorCodeFits reachCode (reachRequest bits first last (suffix cells height bound))
      [divideBoolTag (divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits first last)]
  ...
  have eq : answerExpr.eval final = divideBoolTag (divideReachIndexDFSBool (2^bits) (Raw.check cells height bound) bits first last) :=
    answer_eval rest (2^bits) (finalState cells height bound bits first last)
  rw [eq] at answer
  have result := comp answer (comp loop initial)
  apply result.mono
  have hs := state_space_le rest (Raw.check cells height bound) bits first last (divideEvalFuel (2^bits) bits) hf hl
