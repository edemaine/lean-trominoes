...
/-! # Execution over all last-representative row candidates -/
...
def rows_evalsInTime (rowIndex : Nat) (rows : List (List Bool))
    ...
    (inputEq : data.input = DelimitedBinaryWords.encode ⟨rows⟩ ++ tail)
  ...
  | cons row rows induction =>
      ...
      have rest := induction (rowIndex + 1) afterRow
      ...
      cases selectedEq :
          LastRepresentativeEqualityRows.selected rowIndex row with
      | false =>
          have tokensEq :=
            LastRepresentativeEqualityRowTokens.tokensAux_cons_rejected
              rowIndex row rows selectedEq
          simpa [rowsTime, afterRow, selected, selectedEq, tokensEq]
            using composed
      | true =>
          have tokensEq :=
            LastRepresentativeEqualityRowTokens.tokensAux_cons_selected
              rowIndex row rows selectedEq
          simpa [rowsTime, afterRow, selected, selectedEq, tokensEq,
