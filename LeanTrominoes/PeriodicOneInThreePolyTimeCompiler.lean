...
-/
import LeanTrominoes.PeriodicOneInThreePolyTimeAtoms
import LeanTrominoes.TM2EmptyAlphabetListInputCompiler

/-! # A native flat-output polynomial-time 1D exact-one reduction -/
...
noncomputable local instance exactOneCompilerStackFintype (stack : decider.tm.K) : Fintype (decider.tm.Γ stack) :=
...
noncomputable def fieldsComputableInPolyTime [Inhabited encoding.Γ] :
  ...
  exact finalize_records _ (clauses_nonempty decider symbols)
    (Numeric.width_three _)
    (Numeric.forward _ (PeriodicThreeSATThree.Numeric.forward _
