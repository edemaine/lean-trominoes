...
def referenceIncidences {Variable Element : Type*}
    [DecidableEq Element] (problem : TypedPeriodicThreeDM Variable)
...
def TypedPeriodicThreeDM.encodeIncidence {Variable : Type*}
...
theorem mem_encode_incidences_iff
  ...
  · intro encodedMember
    ...
    have indexLt : tripleIndex < problem.triples.length := by
      simpa [TypedPeriodicThreeDM.encode] using encodedIndexLt
    ...
    · rename_i atomEq
      ...
      have encodedAt :
        ...
        have encodedIndexLt :
          ...
          simpa [TypedPeriodicThreeDM.encode] using indexLt
        have tripleLookup :=
