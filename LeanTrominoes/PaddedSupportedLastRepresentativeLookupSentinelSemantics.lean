...
import LeanTrominoes.PaddedSupportedLastRepresentativeLookupSelectionSemantics

/-! # Unary lookup with an aligned sentinel value -/
...
still returns that datum when the aligned values carry one final sentinel. -/
theorem lookups_selectedRows_map_append_value
