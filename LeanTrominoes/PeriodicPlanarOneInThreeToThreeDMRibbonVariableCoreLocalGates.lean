...
/-- The three connector patterns generated by polarity normalization.  Their
local-gate tables have complete endpoint-aware separation from the whole
variable-site core: fixed-red and fixed-blue use the false orientation, while
fixed-green uses the true orientation. -/
def VariableLocalGateTableEndpointClear
    (kind : VariableConnectorKind) (polarity : Bool) : Prop :=
  (polarity = false ∧ kind ≠ .fixedGreen) ∨
    (polarity = true ∧ kind = .fixedGreen)
...
avoids the continuous interiors of every active normalized local gate, in
both directions. -/
...
/-- A finite variable-site route and a normalized gate belonging to a
...
/-- In each polarity-normalized endpoint-clear local table, every core route has full
...
theorem variableSiteRoute_avoids_localGateRoute_of_slot_ne
  ...
  exact variableSiteRoute_avoids_standardVariableLocalGateRoute_of_slot_ne
    data.countPred data.kind data.polarity slot active
    triple routeColor gateColor clear differentSlot
