...
-/
import LeanTrominoes.CompletionLBooleanOwnership
...
theorem boolean_owner_translation (value : Cell → Bool) (delta c : Cell) :
  ...
  have base : ((Cell.add (microOrigin delta) c).1 / 12,(Cell.add (microOrigin delta) c).2 / 6) =
      Cell.add delta (c.1 / 12,c.2 / 6) := by
    apply Prod.ext <;> dsimp [Cell.add,microOrigin] <;> omega
  have x : (Cell.add (microOrigin delta) c).1 % 12 = c.1 % 12 := by
    dsimp [Cell.add,microOrigin]; omega
  have y : (Cell.add (microOrigin delta) c).2 % 6 = c.2 % 6 := by
    dsimp [Cell.add,microOrigin]; omega
