...
/-- Complete relative separation for two actual factor-eight cycle routes.
At shift zero the incidences must be distinct; every nonzero shift is
strictly contact-free, including a translated copy of the same incidence. -/
theorem
    retainedFinalSourceScaledAllCycleRoutes_avoidEachOther_relative
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
  ...
  · subst relativeTranslate
    have separated :=
      retainedFinalSourceScaledAllCycleRoutes_avoidEachOther
        ...
        firstLiteralMember secondLiteralMember
        (zeroShiftDistinct rfl)
    simpa [translatePolyline,
      PeriodicVariablePlacement.translation, Cell.scale] using separated
  · exact
      (retainedFinalSourceScaledAllCycleRoutes_strictlyAvoidEachOther_of_nonzero
...
/-- Complete relative separation in the public appended-cycle route-family
interface. -/
theorem
    retainedDrawingSourceScaledCoordinatedEightOccurrenceSplitIncidenceRoutes_cycleRoutes_avoidEachOther_relative
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (sourceLocal : formula.IsLocal)
    (sourceWidth : formula.WidthAtMost 3)
  ...
  exact
    retainedFinalSourceScaledAllCycleRoutes_avoidEachOther_relative
      ...
      sourceClausesNonempty firstClauseMember secondClauseMember
      firstLiteralMember secondLiteralMember
      relativeTranslate zeroShiftDistinct
