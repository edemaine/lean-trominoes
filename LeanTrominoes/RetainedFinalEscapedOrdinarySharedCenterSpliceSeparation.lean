...
/-!
# Mixed escaped/ordinary source splices with externally separated fans

The retained source geometry also assembles an escaped first boundary and
an ordinary second boundary at a shared variable center, once their two
complete outer fans are known to be strictly separated.
-/
...
/-- A source-scaled escaped splice strictly avoids a source-scaled ordinary
splice when the discarded final segments are axis-aligned and the escaped
and ordinary complete fans are strictly separated. -/
theorem
  ...
  have firstPrefixAvoidSecondOrdinary :=
    ...
    retainedAngularFanEscapedOrdinarySplicedBoundaryPolylines_strictlyAvoid
      ...
      (by
        simpa [List.getLastD_eq_getLast?, secondLast,
          scalePolyline_getLastD] using firstPrefixAvoidSecondOrdinary)
      (by
