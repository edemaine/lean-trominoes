...
-/
import LeanTrominoes.CompletionLCoreSeparation
...
theorem Atom.motif_inside (a : Atom) {p : Placement Unit} (hp : p ∈ a.motif) :
    p.cells (fun _ => Tromino.L.cells) ⊆ a.pattern.region := by
  intro c hc
  exact a.fixed_inside ((a.fixed_mem c).mpr ⟨p,hp,hc⟩)

theorem shift_cells_twice (p : Placement Unit) (u v : Cell) :
    ((p.shift v).shift u).cells (fun _ => Tromino.L.cells) =
      (p.shift (Cell.add u v)).cells (fun _ => Tromino.L.cells) := by
...
theorem shifted_motif_inside (location : Cell) (entry : Atom × Cell)
    ...
    (p.shift (atomOffset location entry)).cells (fun _ => Tromino.L.cells) ⊆
  ...
  have hd : d ∈ p.cells (fun _ => Tromino.L.cells) := by
...
theorem prescribed_atom (palette : Cell → Fin 24) {f : Finset Cell}
    ...
    ∃ location : Cell, ∃ entry ∈ layout (palette location), ∃ p ∈ entry.1.motif,
      f = (p.shift (atomOffset location entry)).cells (fun _ => Tromino.L.cells) := by
