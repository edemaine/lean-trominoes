...
-/
import LeanTrominoes.CompletionILocalRestriction
...
theorem atom_prescribed_recenter (location : Cell) (entry : Atom × Cell) :
    (fun f => f.image (Cell.add (Cell.sub (0,0) (atomOffset location entry)))) ''
      ...
      (Tromino.I.finiteFootprints entry.1.pattern.prefill : Set (Finset Cell)) := by
  ext f
  constructor
  ...
  · intro hf
    ...
    refine ⟨(p.shift (atomOffset location entry)).cells (fun _ => Tromino.I.cells),
