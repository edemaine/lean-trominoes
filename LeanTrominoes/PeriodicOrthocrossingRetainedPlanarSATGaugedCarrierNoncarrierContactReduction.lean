...
/-- Cancel a carrier occurrence's final lattice translate at a literal
endpoint equality.  This does not use closed segment containment, so it also
applies when the translated noncarrier segment is diagonal. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.exists_anchorAlignedPhysical_endpointEquality_of_final
    {Variable : Type*} [DecidableEq Variable]
    {formula : PeriodicCNF Variable}
    {firstIndexed secondIndexed : IndexedGridSegment}
    {firstShift secondShift point : Cell}
    ...
    (endpointEq :
      point =
        ...
        point =
          (secondIndexed.segment.translate
            ((retainedDeduplicatedGaugedWrappedDrawingPeriodicPlanarSATIncidenceDrawing
              formula).periodTranslation secondShift)).finish) :
    ∃ alignedPoint,
      firstIndexed.segment.InteriorContains alignedPoint ∧
        (alignedPoint =
            (second.physicalSegment.translate
              (carrierMacroPeriodTranslation formula.incidenceGraph
                (Cell.sub second.physicalShift firstShift))).start ∨
          alignedPoint =
            (second.physicalSegment.translate
              (carrierMacroPeriodTranslation formula.incidenceGraph
                (Cell.sub second.physicalShift firstShift))).finish) := by
  let firstRepresentative :=
    first.toCommonShiftRepresentative
  let secondRepresentative :=
    second.toCommonShiftRepresentative
  rw [firstRepresentative.segmentEq] at firstContains
  rw [secondRepresentative.segmentEq] at endpointEq
  let placement :=
    retainedGaugedWrappedDrawingPeriodicPlanarSATPlacement formula
  change
    (first.physicalSegment.translate
  ...
  change
    point =
        (second.physicalSegment.translate
          (placement.translation second.physicalShift)).start ∨
      point =
        (second.physicalSegment.translate
          (placement.translation second.physicalShift)).finish at endpointEq
  let anchorShift := Cell.neg first.sourceClauseAnchor
  let targetShift := Cell.sub second.physicalShift firstShift
  have firstRemainingEq :
      Cell.sub first.physicalShift anchorShift = firstShift := by
  ...
  have secondAlignedEq :
    ...
    simpa only [secondRemainingEq] using
      (segment_translate_placement_add_sub
        ...
        second.physicalShift targetShift)
  rw [← firstAlignedEq] at firstContains
  rw [← secondAlignedEq] at endpointEq
  let commonOffset := placement.translation firstShift
  let alignedPoint := Cell.sub point commonOffset
  have alignedFirst :
      (first.physicalSegment.translate
    ...
    simpa [commonOffset, alignedPoint, Cell.add, Cell.sub] using
      firstContains
  have alignedEndpointEq :
      alignedPoint =
          (second.physicalSegment.translate
            (placement.translation targetShift)).start ∨
        alignedPoint =
          (second.physicalSegment.translate
            (placement.translation targetShift)).finish := by
    rcases endpointEq with endpointEq | endpointEq
    · apply Or.inl
      rcases point with ⟨pointX, pointY⟩
      simp only [alignedPoint, commonOffset, GridSegment.translate,
        Cell.sub, Cell.add, Prod.mk.injEq] at endpointEq ⊢
      constructor <;> omega
    · apply Or.inr
      rcases point with ⟨pointX, pointY⟩
      simp only [alignedPoint, commonOffset, GridSegment.translate,
  ...
  refine ⟨alignedPoint, ?_, ?_⟩
  · simpa only [firstEq] using alignedFirst
  · simpa only [targetShift, targetOffsetEq] using alignedEndpointEq

/-- Cancel a carrier occurrence's final lattice translate at the reverse
asymmetric endpoint contact.  The resulting common point lies in the closed
carrier segment and in the relative interior of the translated noncarrier
segment. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.exists_anchorAlignedPhysical_reverseContact_of_final
    {Variable : Type*} [DecidableEq Variable]
    {formula : PeriodicCNF Variable}
    {firstIndexed secondIndexed : IndexedGridSegment}
    {firstShift secondShift point : Cell}
    ...
    (firstContains :
      (firstIndexed.segment.translate
        ((retainedDeduplicatedGaugedWrappedDrawingPeriodicPlanarSATIncidenceDrawing
          formula).periodTranslation firstShift)).Contains point)
    (secondContains :
      (secondIndexed.segment.translate
        ((retainedDeduplicatedGaugedWrappedDrawingPeriodicPlanarSATIncidenceDrawing
          formula).periodTranslation secondShift)).InteriorContains point) :
    ∃ alignedPoint,
      firstIndexed.segment.Contains alignedPoint ∧
        (second.physicalSegment.translate
          (carrierMacroPeriodTranslation formula.incidenceGraph
            (Cell.sub second.physicalShift firstShift))).InteriorContains
          alignedPoint := by
  let firstRepresentative :=
    first.toCommonShiftRepresentative
  let secondRepresentative :=
  ...
  change
    (first.physicalSegment.translate
      (placement.translation first.physicalShift)).Contains
        point at firstContains
  change
    (second.physicalSegment.translate
      (placement.translation second.physicalShift)).InteriorContains
        point at secondContains
  let anchorShift := Cell.neg first.sourceClauseAnchor
  let targetShift := Cell.sub second.physicalShift firstShift
  have firstRemainingEq :
  ...
  have alignedFirst :
      (first.physicalSegment.translate
        (placement.translation anchorShift)).Contains
          alignedPoint := by
    apply
      (PeriodicGridDrawing.contains_translate_iff
        (first.physicalSegment.translate
  ...
  have alignedSecond :
      (second.physicalSegment.translate
        (placement.translation targetShift)).InteriorContains
          alignedPoint := by
    apply
      (PeriodicGridDrawing.interiorContains_translate_iff
        (second.physicalSegment.translate
...
/-- At a literal carrier-interior/noncarrier-endpoint equality, the
anchor-normalized carrier supporting segment still passes through the
translated center of the noncarrier macrocell, without requiring the
noncarrier segment to be axis-aligned. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.anchorNormalizedCarrier_supportingSegment_contains_translatedMacrocellCenter_of_endpointEquality
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have secondBounds :=
    second.translatedPhysicalSegment_endpoints_in_macrocell
      ...
      center centerEq sourceShift
  rcases
      first.exists_anchorAlignedPhysical_endpointEquality_of_final
        second firstContains endpointEq with
    ⟨alignedPoint, alignedFirst, alignedEndpoint⟩
  have firstPointBounded :=
    inClosedGridRectangle_of_segment_contains
      firstBounds.1 firstBounds.2
      (GridSegment.contains_of_interiorContains alignedFirst)
      ...
      InClosedGridRectangle
        ...
        (planarSATMacrocellRouteUpper
          (Cell.add center
            ((drawing graph).periodTranslation sourceShift)))
        alignedPoint := by
    rcases alignedEndpoint with startEq | finishEq
    · rw [startEq]
      exact secondBounds.1
    · rw [finishEq]
      exact secondBounds.2
  have notSeparated :
      ¬ClosedGridRectanglesSeparated
        (drawingCompleteCarrierLinkRectangleLower
          graph translatedLink)
...
/-- At the reverse asymmetric noncarrier-interior/carrier-closed contact,
the anchor-normalized carrier supporting segment still passes through the
translated center of the noncarrier macrocell. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.anchorNormalizedCarrier_supportingSegment_contains_translatedMacrocellCenter_of_reverseEndpointContact
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have secondBounds :=
    second.translatedPhysicalSegment_endpoints_in_macrocell
      ...
      center centerEq sourceShift
  rcases
      first.exists_anchorAlignedPhysical_reverseContact_of_final
        second firstContains secondContains with
    ⟨alignedPoint, alignedFirst, alignedSecond⟩
  have firstPointBounded :=
    inClosedGridRectangle_of_segment_contains
      firstBounds.1 firstBounds.2 alignedFirst
  have secondPointBounded :=
    inClosedGridRectangle_of_segment_contains
      secondBounds.1 secondBounds.2
      (GridSegment.contains_of_interiorContains alignedSecond)
  have notSeparated :
      ¬ClosedGridRectanglesSeparated
        (drawingCompleteCarrierLinkRectangleLower
          graph translatedLink)
...
/-- A literal noncarrier endpoint equality inside a lifted vertex macrocell
gives the same neighboring source-coordinate bound, without an
axis-alignment hypothesis on the noncarrier segment. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.carrier_liftedVertex_relative_neighbor_of_endpointEquality
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have centerContains :
    ...
    rw [← targetCenterEq]
    simpa only [graph, anchorShift, sourceShift, translatedLink] using
      (first.anchorNormalizedCarrier_supportingSegment_contains_translatedMacrocellCenter_of_endpointEquality
        formula wellFormed degree isLocal second
        link firstComponentEq secondNotCarrier
        (liftedIncidenceVertexPosition
          formula vertex vertexTranslate)
        centerEq firstContains endpointEq)
  let relative :=
    Cell.sub translatedLink.first.translate targetVertexTranslate
  have normalizedContains :
      (translatedLink.first.indexed.segment.translate
...
/-- A reverse asymmetric noncarrier-interior/carrier-closed contact inside a
lifted vertex macrocell gives the same neighboring source-coordinate bound. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.carrier_liftedVertex_relative_neighbor_of_reverseEndpointContact
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
...
/-- Every final segment selected from a carrier source remains
axis-aligned after anchor normalization. -/
theorem
    FinalGaugedSegmentOccurrenceWitness.indexedSegment_isAxisAligned_of_carrier
    {Variable : Type*} [DecidableEq Variable]
    {formula : PeriodicCNF Variable}
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have orthogonal :
    ...
    exact
      (retainedDrawingPlanarSATCarrierLensIncidenceDrawing_isValid
        wellFormed degree isLocal selectedMember).2.1
  have clauseMember :=
    witness.routeWitness.metadata.retainedLocalClauseMember
      wellFormed degree isLocal witness.metadata_retainedValid
  have physicalAligned :
      witness.physicalSegment.IsAxisAligned := by
    apply
      (witness.routeWitness.metadata.source.incidenceDrawing formula)
        |>.embeddedSegment_isAxisAligned_of_members
          orthogonal clauseMember witness.routeWitness.literalMember
    exact
      List.fst_mem_of_mem_zipIdx
        witness.physicalSegment_mem_sourceRoute
  rw [witness.indexedSegment_eq_anchorNormalizedPhysical]
  exact
    (GridSegment.isAxisAligned_translate
      witness.physicalSegment
      (carrierMacroPeriodTranslation formula.incidenceGraph
        (Cell.neg witness.sourceClauseAnchor))).mpr
      physicalAligned
...
/-- The common-shift raw-carrier bridge with component-equivalent
noncarrier reindexing also keeps the carrier interior away from both
endpoints of the noncarrier segment. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_carrier_noncarrier_of_common_raw_retainedOrbitCondition
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have routeAvoid :=
    first.routeWitness.metadata
      |>.two_periodTranslate_localRoutes_avoidEachOther_of_raw_carrier_retainedOrbitCondition
        ...
        second.routeWitness.literalMember secondNotCarrier
  apply
    retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_two_periodTranslate_localRoutesAvoidEachOther
      formula first second firstSourceShift secondSourceShift
      commonShiftEq routeAvoid
  exact firstContains
...
/-- Endpoint-equality balanced raw-carrier separation when the noncarrier
orbit condition has one occurrence coordinate.  Unlike closed containment,
the endpoint hypothesis does not require the noncarrier segment to be
axis-aligned. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_carrier_noncarrier_of_balanced_source_coordinate
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  have avoidEndpoints
    ...
    rcases
        exists_neighbor_balancing_adjustments
          ...
          (by
            simpa only [firstBase, relative] using
              contactClose point firstContains endpointEq) with
      ⟨firstAdjustment, secondAdjustment,
        adjustmentDifference,
        translatedFirstNeighbor,
        translatedSecondNeighbor⟩
    ...
    exact
      retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_carrier_noncarrier_of_common_raw_retainedOrbitCondition
        ...
        link firstComponentEq secondNotCarrier
        translatedLinkMember targetFirstNeighbor
        translatedSecondCondition point firstContains
  constructor
  · intro startEq
    exact (avoidEndpoints (Or.inl startEq)).1 startEq
  · intro finishEq
    exact (avoidEndpoints (Or.inr finishEq)).2 finishEq

/-- Reverse endpoint-aware balanced raw-carrier separation when the
noncarrier orbit condition has one occurrence coordinate. -/
theorem
  ...
  rcases
      exists_neighbor_balancing_adjustments
        ...
        (by
          simpa only [firstBase, relative] using
            contactClose point secondContains firstContains) with
    ⟨firstAdjustment, secondAdjustment,
      adjustmentDifference,
      translatedFirstNeighbor,
      translatedSecondNeighbor⟩
...
/-- Endpoint-equality balanced raw-carrier separation when two noncarrier
occurrence coordinates share one source adjustment. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_carrier_noncarrier_of_balanced_two_source_coordinates
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
        ...
        point ≠
          (secondIndexed.segment.translate
            ((retainedDeduplicatedGaugedWrappedDrawingPeriodicPlanarSATIncidenceDrawing
              formula).periodTranslation secondShift)).finish := by
    have contactBounds :=
      contactClose point firstContains endpointEq
    rcases
        exists_neighbor_balancing_adjustments_two_second_bases
          firstBase secondFirstBase secondSecondBase relative
          (by simpa only [firstBase, relative] using contactBounds.1)
...
/-- Reverse endpoint-aware balanced raw-carrier separation when the
noncarrier orbit condition has two occurrence coordinates sharing one
source adjustment. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_noncarrier_carrier_of_balanced_two_source_coordinates
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  let relative := Cell.sub firstShift secondShift
  intro firstContains
  have contactBounds := contactClose point secondContains firstContains
  rcases
      exists_neighbor_balancing_adjustments_two_second_bases
        firstBase secondFirstBase secondSecondBase relative
        (by simpa only [firstBase, relative] using contactBounds.1)
...
/-- Final carrier segment interiors avoid both endpoints of every
routed-clause segment occurrence, including a diagonal stored segment. -/
theorem
  ...
  · intro adjustment adjustmentNeighbor
    ...
    rw [translatedSiteEq]
    simpa [Cell.add] using adjustmentNeighbor
  · intro contactPoint contactFirst endpointEq
    rcases
        first.routeWitness.metadata.source
          |>.exists_eq_carrier_of_component_eq
            link firstComponentEq with
    ...
    have relativeNeighbor :
      ...
      exact
        (mem_neighborTranslations_iff _).mp
          (PeriodicGridDrawing.relativeTranslate_isNeighbor_of_endpointEquality_of_secondHalfOpen
            firstEndpointBounds secondEndpointBounds
            contactFirst endpointEq)
    have sumMember :=
      firstBaseNeighbor.add_mem_doubleNeighborTranslations
        relativeNeighbor
    simpa [Cell.sub] using sumMember

/-- Routed-clause segment interiors avoid every closed final carrier segment
occurrence, including endpoint-only contacts. -/
theorem
  ...
  · intro contactPoint contactSecond contactFirst
    ...
    have relativeNeighbor :
      ...
      exact
        (mem_neighborTranslations_iff _).mp
          (PeriodicGridDrawing.relativeTranslate_isNeighbor_of_reverseEndpointContact_of_secondHalfOpen
            firstEndpointBounds secondEndpointBounds
            contactSecond contactFirst)
    have sumMember :=
      firstBaseNeighbor.add_mem_doubleNeighborTranslations
        relativeNeighbor
    simpa [Cell.sub] using sumMember
...
/-- Carrier segment interiors avoid both endpoints of every crossover
segment occurrence.  The two crossing-coordinate representatives remain
coupled through the same source adjustment. -/
theorem
  ...
  refine
    retainedDeduplicatedGaugedWrappedDrawing_endpointsAvoidInterior_of_carrier_noncarrier_of_balanced_two_source_coordinates
      ...
      (Cell.add crossing.secondTranslate
        (Cell.neg second.sourceClauseAnchor))
      crossingBasesClose ?_ ?_ point firstContains
  · intro adjustment firstNeighbor secondNeighbor
    rw [secondSourceEq]
    constructor
    · simpa [Cell.add, add_assoc] using firstNeighbor
  ...
  · intro contactPoint contactFirst endpointEq
    have carrierContains :=
      first.anchorNormalizedCarrier_supportingSegment_contains_translatedMacrocellCenter_of_endpointEquality
        ...
        link firstComponentEq secondNotCarrier
        crossing.point centerEq
        (point := contactPoint) contactFirst endpointEq
    have carrierContainsTranslated :
        (translatedLink.first.indexed.segment.translate
          ((drawing graph).periodTranslation
            translatedLink.first.translate)).Contains
...
/-- Reverse endpoint-aware crossover--carrier separation follows from
doubled-halo bounds to both coupled crossing occurrences. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_crossover_carrier_of_contact_close
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  refine
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_noncarrier_carrier_of_balanced_two_source_coordinates
      ...
      (Cell.add crossing.secondTranslate
        (Cell.neg second.sourceClauseAnchor))
      crossingBasesClose ?_ contactClose point secondContains
  intro adjustment firstNeighbor secondNeighbor
  rw [secondSourceEq]
  constructor
  · simpa [Cell.add, add_assoc] using firstNeighbor
...
/-- Crossover segment interiors avoid closed carrier segment occurrences at
arbitrary quotient translates. -/
theorem
...
/-- Endpoint-aware carrier--bend separation follows from the same
doubled-halo bound to the bend occurrence. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_carrier_bend_of_contact_close
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
  ...
  refine
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_carrier_noncarrier_of_balanced_source_coordinate
      ...
      (Cell.add routeBend.translate
        (Cell.neg second.sourceClauseAnchor))
      ?_ contactClose point firstContains
  intro adjustment adjustmentNeighbor
  rw [secondSourceEq]
  change
    IsNeighborTranslation
...
/-- Carrier segment interiors avoid closed bend segment occurrences at
arbitrary quotient translates. -/
theorem
...
/-- Carrier segment interiors avoid both endpoints of every bend segment
occurrence, without assuming that the selected bend route segment itself is
axis-aligned. -/
theorem
...
/-- Endpoint-aware carrier--routed-variable separation follows from the same
balanced represented-route occurrence. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_carrier_routedVariable_of_contact_close
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
...
/-- Final carrier segment interiors avoid closed represented routed-variable
arm occurrences at arbitrary quotient translates. -/
theorem
  ...
  intro contactPoint contactFirst contactSecond
  have relativeSiteNeighbor :=
    first.carrier_liftedVertex_relative_neighbor_of_endpointContact
      formula wellFormed degree isLocal second
      carrierLink firstComponentEq secondNotCarrier
      (.variable site.1)
      (drawingVariableRouteSite_vertex_mem formula siteMember)
      site.2 centerEq contactFirst contactSecond
  have sumMember :=
    relativeSiteNeighbor.add_mem_doubleNeighborTranslations
      offsetNeighbor
  have desiredEq :
...
/-- Final carrier segment interiors avoid both endpoints of every
represented routed-variable arm occurrence. -/
theorem
  ...
  · intro contactPoint contactFirst endpointEq
    have relativeSiteNeighbor :=
      first.carrier_liftedVertex_relative_neighbor_of_endpointEquality
        formula wellFormed degree isLocal second
        carrierLink firstComponentEq secondNotCarrier
        (.variable site.1)
        (drawingVariableRouteSite_vertex_mem formula siteMember)
        site.2 centerEq contactFirst endpointEq
    have sumMember :=
      relativeSiteNeighbor.add_mem_doubleNeighborTranslations
        offsetNeighbor
    have desiredEq :
...
/-- Reverse endpoint-aware routed-variable--carrier separation follows from
the same balanced represented-route occurrence. -/
theorem
    retainedDeduplicatedGaugedWrappedDrawing_avoidsInterior_of_routedVariable_carrier_of_contact_close
    {Variable : Type*} [DecidableEq Variable]
    (formula : PeriodicCNF Variable)
    (wellFormed : formula.incidenceGraph.IsWellFormed)
    (degree : formula.incidenceGraph.DegreeAtMost 3)
...
/-- Represented routed-variable arm interiors avoid closed final carrier
segments at arbitrary quotient translates. -/
theorem
  ...
  intro contactPoint contactSecond contactFirst
  have relativeSiteNeighbor :=
    first.carrier_liftedVertex_relative_neighbor_of_reverseEndpointContact
      formula wellFormed degree isLocal second
      carrierLink firstComponentEq secondNotCarrier
      (.variable site.1)
      (drawingVariableRouteSite_vertex_mem formula siteMember)
