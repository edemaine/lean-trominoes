...
-/
import LeanTrominoes.CompletionLBoundaryNeighbors
...
def booleanMicroOwner (value : Cell → Bool) (c : Cell) : Cell :=
  let i : Cell := (c.1 / 12,c.2 / 6)
  let x := c.1 % 12
  if c.2 % 6 = 0 ∧
      (if x = 2 then value i = false else if x = 8 then value i = true else x < 6)
...
theorem boolean_owner_portal (value : Cell → Bool) (i : Cell) (right : Bool) :
  ...
  have base : ((portalCell i right).1 / 12,(portalCell i right).2 / 6) = i := by
    cases right <;> apply Prod.ext <;> dsimp [portalCell] <;> omega
  have y : (portalCell i right).2 % 6 = 0 := by dsimp [portalCell]; omega
  have x : (portalCell i right).1 % 12 = if right then 8 else 2 := by
