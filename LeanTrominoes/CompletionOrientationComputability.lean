...
open Gadget Gadget.PeriodicOrthogonalDrawing Computability TrominoAssignment
...
private theorem square_origin_primrec : Primrec fun c => origin (brickOfSquare c) := by
  ...
  exact Primrec.pair
    (int_add_primrec.comp (int_multiply_primrec.comp (Primrec.const 24) Primrec.fst)
      (int_multiply_primrec.comp (Primrec.const 12) diff))
    (int_multiply_primrec.comp (Primrec.const 36) diff)
