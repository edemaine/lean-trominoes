...
-/
import LeanTrominoes.PeriodicExactOneFieldCyclePair
...
theorem cyclePredicate_eq (f : PeriodicCNF Nat) (bits : Nat) :
    cyclePredicate f bits = cycleSearchIndexDFSBoolAtDepth (2^bits) bits (ExactOneFieldPredicate.check f) := by
...
theorem cycle_eval (f : PeriodicCNF Nat) (bits : Nat) :
    cycleCode.eval (cycleValues bits (suffix f)) =
      pure [(cycleSearchIndexDFSBoolAtDepth (2^bits) bits (ExactOneFieldPredicate.check f)).toNat] := by
...
theorem cycle_fits (f : PeriodicCNF Nat) (bits : Nat) :
    EvaluatorCodeFits cycleCode (cycleValues bits (suffix f))
      [(cycleSearchIndexDFSBoolAtDepth (2^bits) bits (ExactOneFieldPredicate.check f)).toNat]
