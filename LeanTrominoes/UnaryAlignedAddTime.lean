...
-/
import LeanTrominoes.UnaryAlignedAddInterface
import LeanTrominoes.UnaryAlignedAddTimeBound

/-! # Polynomial-time aligned unary addition -/
...
/-- Pointwise addition of aligned unary fields is computable in linear time
in the complete separated encoding. -/
noncomputable def computableInPolyTime :
    @TM2ComputableInPolyTime
      Input (List Nat) InputSymbol UnarySymbol
      encode UnaryFieldEncoderMachine.unaryFields
      (fun input => sums input.firsts input.seconds) where
  tm := machine
