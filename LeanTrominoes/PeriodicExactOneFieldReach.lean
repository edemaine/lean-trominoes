...
-/
import LeanTrominoes.PeriodicExactOneFieldSavitchRun
import LeanTrominoes.PolyominoStripReachInput

/-! # A complete certified exact-one reachability program -/
...
theorem reach_eval (f : PeriodicCNF Nat) (bits first last : Nat) :
    reachCode.eval (reachRequest bits first last (suffix f)) =
      pure [divideBoolTag (divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits first last)] := by
  ...
  have loopRun := DivideEvalPartrec.flatIterate_stepCode_eval_suffix baseCode 0 (2^bits)
    (ExactOneFieldPredicate.check f) (suffix f)
...
theorem reach_fits (f : PeriodicCNF Nat) (bits first last : Nat)
    ...
    EvaluatorCodeFits reachCode (reachRequest bits first last (suffix f))
      [divideBoolTag (divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits first last)]
  ...
  have eq : answerExpr.eval final = divideBoolTag (divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits first last) :=
    answer_eval rest (2^bits) (finalState f bits first last)
  rw [eq] at answer
  have result := comp answer (comp loop initial)
  apply result.mono
  have hs := state_space_le rest (ExactOneFieldPredicate.check f) bits first last (divideEvalFuel (2^bits) bits) hf hl
