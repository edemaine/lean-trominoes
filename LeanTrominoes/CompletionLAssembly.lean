...
-/
import LeanTrominoes.CompletionLCanonicalStates
import LeanTrominoes.TrominoCompletionAssembly

/-! # Global L-completion as compatible local gadget states -/
...
theorem atom_prescribed_translate (location : Cell) (entry : Atom × Cell) :
    (fun f => f.image (Cell.add (atomOffset location entry))) ''
      (Tromino.L.finiteFootprints entry.1.pattern.prefill : Set (Finset Cell)) =
      atomPrescribed location entry := by
  ext f
  constructor
  · rintro ⟨g,hg,rfl⟩
    obtain ⟨p,hp,rfl⟩ := Finset.mem_image.mp hg
    exact ⟨p,(entry.1.prefill_mem p).mp hp,(Placement.shift_cells_image p .L _).symm⟩
  · rintro ⟨p,hp,rfl⟩
    exact ⟨p.cells (fun _ => Tromino.L.cells),
      Finset.mem_image.mpr ⟨p,(entry.1.prefill_mem p).mpr hp,rfl⟩,
      (Placement.shift_cells_image p .L _).symm⟩
...
theorem state_completion {palette : Cell → Fin 24} (o : Occurrence palette) (outside : Finset Cell)
    ...
    Tromino.L.Completable (stateTarget o outside : Set Cell) (atomPrescribed o.location o.entry) := by
