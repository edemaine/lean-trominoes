...
-/
import LeanTrominoes.CompletionLBrickRegions
import Mathlib.Data.Int.Interval

/-! # The closed twelve-by-six microcells underlying the L-brick layout -/
...
def microShape : Finset Cell :=
  (Finset.Ico (0 : Int) 12 ×ˢ Finset.Ico (1 : Int) 6) ∪
    (Finset.Ico (6 : Int) 12 ∪ {2}).image (fun x => (x,0)) ∪
    (Finset.Ico (0 : Int) 6 ∪ {8}).image (fun x => (x,6))

theorem micro_shape_eq : LEqBoundary.pattern.region = microShape := by decide +kernel
...
def microOrigin (i : Cell) : Cell := (12 * i.1,6 * i.2)

def microRegion (i : Cell) : Finset Cell := LEqBoundary.pattern.region.image (Cell.add (microOrigin i))

theorem mem_microRegion (i c : Cell) :
    c ∈ microRegion i ↔ MemberAt (c.1 - 12 * i.1) (c.2 - 6 * i.2) := by
  have offset : c ∈ microRegion i ↔ Cell.sub c (microOrigin i) ∈ LEqBoundary.pattern.region := by
    constructor
    · intro hc
...
theorem micro_cover (c : Cell) : ∃ i : Cell, c ∈ microRegion i := by
  by_cases boundary : c.2 % 6 = 0 ∧ c.1 % 12 < 6
  · refine ⟨(c.1 / 12,c.2 / 6 - 1),?_⟩
    rw [mem_microRegion]
    dsimp
    unfold MemberAt
    omega
  · refine ⟨(c.1 / 12,c.2 / 6),?_⟩
    rw [mem_microRegion]
    dsimp
    unfold MemberAt
    omega
...
theorem micro_overlap {i j c : Cell} (hi : c ∈ microRegion i) (hj : c ∈ microRegion j) :
    i = j ∨ (i.1 = j.1 ∧ (c.1 = 12 * i.1 + 2 ∨ c.1 = 12 * i.1 + 8) ∧
      ((i.2 + 1 = j.2 ∧ c.2 = 6 * j.2) ∨ (j.2 + 1 = i.2 ∧ c.2 = 6 * i.2))) := by
