...
/-! # Execution of a representative-row suffix scan -/
...
/-- Copy the untested suffix and its closing delimiter onto the row stack. -/
def scanSuffixBits_evalsInTime (representative : Bool) (bits : List Bool)
  ...
  | cons bit bits induction =>
      ...
      have composed := EvalsToInTime.trans (TM2.step program)
        ...
        (scanSuffixCfg representative nextData)
        (some (finishRowCfg representative
          { nextData with
            input := tail
            rowReverse :=
