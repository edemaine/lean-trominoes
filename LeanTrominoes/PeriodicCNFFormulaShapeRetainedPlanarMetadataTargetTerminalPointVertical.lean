...
/-! # Vertical drawing-point quotients of target terminals -/
...
/-- The vertical period quotient of a target-terminal drawing point is the
incidence edge's vertical offset. -/
theorem targetTerminal_drawingPoint_vertical_ediv_eq_edgeOffset
    ...
    (SegmentTerminal.drawingPoint source.incidenceGraph
      ((⟨incidence, edgeIndex, (0, 0)⟩ :
        CNFRouteOccurrence Variable).targetTerminal source)).2 /
  ...
  change
    (SegmentTerminal.drawingPoint source.incidenceGraph
      (occurrence.targetTerminal source)).2 /
        drawingGridSize source.incidenceGraph =
  ...
  have twoSmall : (2 : Int) < drawingGridSize source.incidenceGraph := by
    simp only [drawingGridSize]
    omega
  exact coordinate_add_period_mul_ediv_eq_shift periodPositive
    (by norm_num) twoSmall _
