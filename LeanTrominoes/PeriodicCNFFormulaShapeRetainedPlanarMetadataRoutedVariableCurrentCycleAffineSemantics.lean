...
/-! # Affine semantics of current-slice routed-variable cycle pairs -/
...
@[simp] theorem routedVariableCurrentCycleAffinePredicate_evalPair
    (pair : RouteDescriptor × RouteDescriptor) :
    routedVariableCurrentCycleAffinePredicate.evalPair pair =
      routedVariableCurrentCyclePair pair := by
  rcases pair with ⟨first, second⟩
  simp only [routedVariableCurrentCycleAffinePredicate, all,
          ...
          (decide
            ...
            decide
              ((verticalOffset .second).evalPair (first, second) =
                (constant 0).evalPair (first, second)))))) =
      routedVariableCurrentCyclePair (first, second)
  ...
  simp [constant, Expression.evalPair, Expression.eval,
    routedVariableCurrentCyclePair, descriptorAt, Prod.ext_iff, rankTwo]
