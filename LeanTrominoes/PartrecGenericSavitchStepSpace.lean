...
/-! # A reusable evaluator-space certificate for the Savitch driver

The transition leaf, its data suffix, and its space certificate are parameters.
The structural driver proof therefore applies to variable-tile strip search.
-/

namespace LeanTrominoes.FiniteState.GenericSavitchStep
open Computability Turing Turing.PartrecToTM2 Turing.PartrecToTM2.EvaluatorCodeFits
...
@[simp] def flatProgramList (suffix : List Nat) (context stateCount : Nat) (state : DivideEvalState) : List Nat :=
  divideEvalProgramList context stateCount state ++ suffix

def stepSpaceUnit (suffix : List Nat) (baseCost : Nat → Nat → DivideEvalState → Nat)
    (context stateCount : Nat) (state : DivideEvalState) : Nat :=
  encodedListSpace (flatProgramList suffix context stateCount state) + baseCost context stateCount state + 1

def stepCost (suffix : List Nat) (baseCost : Nat → Nat → DivideEvalState → Nat)
    (context stateCount : Nat) (state : DivideEvalState) : Nat :=
  1000000000000000000000000000000 * stepSpaceUnit suffix baseCost context stateCount state

set_option maxRecDepth 100000 in
set_option maxHeartbeats 2000000 in
theorem exactStep
    (suffix : List Nat) (baseCode : ToPartrec.Code) (relation : Nat → Nat → Bool)
    (baseCost : Nat → Nat → DivideEvalState → Nat)
    (baseFit : ∀ context stateCount state,
      EvaluatorCodeFits baseCode (flatProgramList suffix context stateCount state)
        [divideBoolTag (decide (state.query.first = state.query.last) ||
          relation state.query.first state.query.last)] (baseCost context stateCount state))
    ...
    Turing.PartrecToTM2.EvaluatorCodeFits
        (FiniteState.DivideEvalPartrec.stepCode
          baseCode)
        (flatProgramList suffix
          context stateCount state)
        (flatProgramList suffix
          context stateCount
          (FiniteState.divideEvalStep stateCount
            (relation) state))
        (stepCost suffix baseCost context stateCount state) := by
  cases state with
    ...
    | mk depth first last =>
      ...
      | none =>
        ...
        | zero =>
          ...
          have base := baseFit context stateCount state
          let baseResult :=
          ...
          let tag := someBoolField values
            baseCode baseResult
            (baseCost context stateCount state)
          ...
          let unit := stepSpaceUnit suffix baseCost context stateCount state
          have valuesBound : encodedListSpace values ≤ unit := by
            simp [unit, stepSpaceUnit, values, state]
            omega
          have baseCostBound :
              baseCost context stateCount state ≤ unit := by
          ...
          have tagCostBound : tag.cost ≤ 100000000 * (unit + 1) := by
            simpa [tag, someBoolField] using
              someBoolTagCost_le_budget values baseResult
                (baseCost context stateCount state) unit
          ...
          all_goals
            ...
            · rw [show stepCost suffix baseCost context stateCount state =
        ...
        | succ depth =>
          ...
          | zero =>
            ...
            let unit := stepSpaceUnit suffix baseCost context 0 state
            have valuesBound : encodedListSpace values ≤ unit := by
              simp [unit, stepSpaceUnit, values, state]
              omega
            ...
            have zeroBits : (Computability.encodeNat 0).length = 0 := rfl
            have oneBits : (Computability.encodeNat 1).length = 1 := rfl
            rw [show stepCost suffix baseCost context 0 state =
                1000000000000000000000000000000 * unit by rfl]
          ...
          | succ middle =>
            ...
            have wholeFits := (by
              ...
              FiniteState.divideOptionBoolTag] using whole)
            apply wholeFits.mono
            let unit := stepSpaceUnit suffix baseCost context (middle + 1) state
            have valuesBound : encodedListSpace values ≤ unit := by
              simp [unit, stepSpaceUnit, values, state]
              omega
            have get0 := getCost_le_budget 0 values unit (by omega) valuesBound
            ...
            have zeroBits : (Computability.encodeNat 0).length = 0 := rfl
            have oneBits : (Computability.encodeNat 1).length = 1 := rfl
            rw [show stepCost suffix baseCost context (middle + 1) state =
                1000000000000000000000000000000 * unit by rfl]
            simp [fieldsCost, branchZeroZeroCost, branchZeroSuccCost,
              branchZeroTestCost, prependCost, values, state, fieldFits,
              getField, succField, zeroField, predecessorField,
...
end LeanTrominoes.FiniteState.GenericSavitchStep
