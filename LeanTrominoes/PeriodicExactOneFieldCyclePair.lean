...
-/
import LeanTrominoes.PeriodicExactOneFieldCycleAdapters
import LeanTrominoes.PartrecBooleanUniformSpace

/-! # A guarded edge-and-return-path test for cycle search -/
...
def pairTransitionCode : Code := ExactOneFieldPredicate.decision.code.comp pairTransitionInputCode
...
def pairPredicate (f : PeriodicCNF Nat) (bits first second : Nat) : Bool :=
  decide (first < 2^bits ∧ second < 2^bits) &&
    (ExactOneFieldPredicate.check f first second &&
      divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits second first)
...
theorem pairTransition_eval (f : PeriodicCNF Nat) (bits first second : Nat) :
    pairTransitionCode.eval (pairValues bits first second (suffix f)) =
      pure [(ExactOneFieldPredicate.check f first second).toNat] := by
  simp [pairTransitionCode,pairTransitionInput_eval,Part.bind_eq_bind,
    decision_code_eval f first second]

theorem pairReach_eval (f : PeriodicCNF Nat) (bits first second : Nat) :
    pairReachCode.eval (pairValues bits first second (suffix f)) =
      pure [(divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits second first).toNat] := by
...
theorem pair_eval (f : PeriodicCNF Nat) (bits first second : Nat) :
  ...
  let a := ExactOneFieldPredicate.check f first second
  let b := divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits second first
...
theorem pair_fits (f : PeriodicCNF Nat) (bits first second : Nat)
  ...
  have body : decide (first < 2^bits ∧ second < 2^bits) = true →
      EvaluatorCodeFits pairBodyCode values
        [(ExactOneFieldPredicate.check f first second &&
          divideReachIndexDFSBool (2^bits) (ExactOneFieldPredicate.check f) bits second first).toNat]
    ...
    have edge' : EvaluatorCodeFits pairTransitionCode values [(ExactOneFieldPredicate.check f first second).toNat]
