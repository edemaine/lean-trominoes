...
-/
import LeanTrominoes.PeriodicOrthocrossingCarrierCanonicalTerminalCoordinateActiveCompiler
...
/-! # Exact canonical terminal positions from numeric descriptor streams -/

noncomputable section

namespace LeanTrominoes.PeriodicOrthocrossing.CanonicalTerminalCoordinateFieldStream
...
private def fields (horizontal keepPositive : Bool)
  ...
  TM2EndDelimitedBlockMap.mappedOutput RouteDescriptorPairFieldTags.isPairEnd
    (fun tokens => UnaryFieldEncoderMachine.unaryFields
      (activeCanonicalTerminalCoordinateFields horizontal keepPositive tokens))
...
private noncomputable def fieldsComputableInPolyTime (horizontal keepPositive : Bool) :
  ...
  let blockCompiler : TM2ComputableInPolyTime id id
      (fun tokens => UnaryFieldEncoderMachine.unaryFields
        (activeCanonicalTerminalCoordinateFields horizontal keepPositive tokens)) :=
    encodedOutputCompiler id (activeCanonicalTerminalCoordinateFields horizontal keepPositive)
      (activeCanonicalTerminalCoordinateFieldsComputableInPolyTime horizontal keepPositive)
...
theorem values_eq_activeNodes (horizontal keepPositive : Bool) (period : Nat)
    ...
    (bounds : ∀ descriptor ∈ descriptors, descriptor.CoordinateBounds) :
    values horizontal keepPositive descriptors =
      ((paddedTerminalCarrierNodeCandidateStream descriptors).filterMap Candidate.value).map
        (carrierNodeCanonicalCoordinateFieldAtPeriod horizontal keepPositive period) := by
  ...
  rw [activeCanonicalTerminalCoordinateFields_descriptorPairTokens horizontal keepPositive pair
    (bounds pair.1 firstMember), periodEq pair.1 firstMember]
...
theorem values_numericRouteDescriptors
  ...
  rw [values_eq_activeNodes horizontal keepPositive
    ...
    (fun _ member => PeriodicCNF.numericRouteDescriptor_coordinateBounds formula wellFormed degree member)]
  rw [paddedTerminalCarrierNodeCandidateStream_numericRouteDescriptors
    formula wellFormed degree isLocal forward]

end LeanTrominoes.PeriodicOrthocrossing.CanonicalTerminalCoordinateFieldStream
