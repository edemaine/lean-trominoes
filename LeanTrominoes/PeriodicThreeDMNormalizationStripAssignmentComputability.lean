...
-/
import LeanTrominoes.PeriodicThreeDMNormalizationStripRasterLocationComputability

/-! # Primitive-recursive rectangular route assignments -/
...
abbrev StripRouteInteriorAssignmentsInput :=
  (Nat × WireColor) × List Cell

/-- Rectangular route-interior rasterization is primitive recursive. -/
theorem stripRouteInteriorAssignments_primrec :
    Primrec fun input : StripRouteInteriorAssignmentsInput =>
      stripRouteInteriorAssignments input.1.1 input.1.2 input.2 := by
  have step : Primrec₂ fun (parameters : Nat × WireColor)
      ...
      if 2 ≤ state.2.1.length then
        (stripRasterLocation parameters.1 (state.2.1.getD 0 (0, 0)),
    ...
    change Primrec fun combined :
      ...
      if 2 ≤ combined.2.2.1.length then
        (stripRasterLocation combined.1.1
    ...
    have location : Primrec fun combined :
        ...
        stripRasterLocation combined.1.1
          (combined.2.2.1.getD 0 (0, 0)) :=
      stripRasterLocation_primrec.comp
        (Primrec.pair
  ...
  have recursion := Primrec.list_rec
    (f := fun input : StripRouteInteriorAssignmentsInput => input.2)
    (g := fun _ => ([] : List NormalizedCellAssignment))
    (h := fun input state =>
      if 2 ≤ state.2.1.length then
        (stripRasterLocation input.1.1 (state.2.1.getD 0 (0, 0)),
          routingCellTypeAt state.1
            (state.2.1.getD 0 (0, 0))
  ...
  exact recursion.of_eq fun input => by
    fun_induction stripRouteInteriorAssignments
      input.1.1 input.1.2 input.2 with
    | case1 before current after rest induction =>
        change
          (stripRasterLocation input.1.1 current,
